...
df["backend"] = df["backend"].str.strip().str.lower().replace({"mojo": "Mojo", "hip": "HIP"})
...
for ax, d, title in zip(axes, [df32, df64], ["Single Precision", "Double Precision"]):
    ...
    ax.text(4 - 0.5, 5500, f'Theoretical Peak = {peak_bw} GB/s',
...
for ax in axes:
    ...
    ax.grid(True, which='major', axis='x', linestyle='--', alpha=0.2, zorder=0)
